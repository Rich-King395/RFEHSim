...
def test_load_default_config() -> None:
    ...
    assert config.simulation.duration_s == pytest.approx(180.0)
    assert config.simulation.dt_s == pytest.approx(0.001)
    assert config.simulation.seed == 42
    assert config.scenario.app_id == "video"
    assert config.scenario.action_id == "play"
    assert config.scenario.distance_m == pytest.approx(0.5)
    assert config.scenario.mobile_devices[0].device_id == "phone_1"
    assert config.scenario.mobile_devices[0].distance_m == pytest.approx(0.5)
    ...
    assert config.transmitter.default_eirp_w == pytest.approx(dbm_to_watt(20.0))
    assert config.transmitter.model == "transaction"
    assert config.transmitter.mobile_eirp_w_mean == pytest.approx(dbm_to_watt(20.0))
    ...
    assert config.transmitter.ap_eirp_dbm_std == pytest.approx(1.0)
    ...
    assert config.harvester.capacitance_f == pytest.approx(0.001)
...
def test_load_multi_mobile_config() -> None:
    """The preferred multi-mobile scenario schema loads typed device settings."""
    config = load_config(Path("configs/multi_mobile_v1.yaml"))

    assert config.scenario.receiver_id == "rfeh_0"
    assert config.scenario.app_id == "video"
    assert config.scenario.action_id == "play"
    assert config.scenario.ap is not None
    assert config.scenario.ap.ap_id == "ap_0"
    assert config.scenario.ap.distance_m == pytest.approx(0.3)
    assert config.scenario.ap.eirp_w_mean == pytest.approx(dbm_to_watt(20.0))
    assert config.scenario.ap.eirp_dbm_std == pytest.approx(1.0)

    devices = config.scenario.mobile_devices
    assert len(devices) == 3
    assert devices[0].device_id == "phone_1"
    assert devices[0].app_id == "video"
    assert devices[0].action_id == "play"
    assert devices[0].distance_m == pytest.approx(0.5)
    assert devices[0].eirp_w_mean == pytest.approx(dbm_to_watt(18.0))
    assert devices[0].eirp_dbm_std == pytest.approx(1.0)
    assert devices[0].start_offset_s == pytest.approx(0.0)
    assert devices[1].device_id == "phone_2"
    assert devices[1].app_id == "game"
    assert devices[1].action_id == "loading"
    assert devices[1].distance_m == pytest.approx(0.5)
    assert devices[1].start_offset_s == pytest.approx(1.5)
    assert devices[2].device_id == "phone_3"
    assert devices[2].app_id == "communication"
    assert devices[2].action_id == "send_text"
    assert devices[2].distance_m == pytest.approx(0.7)
    assert devices[2].eirp_w_mean == pytest.approx(dbm_to_watt(16.0))
    assert devices[2].eirp_dbm_std == pytest.approx(1.0)
    assert devices[2].start_offset_s == pytest.approx(3.0)
    assert config.scenario.source_distances_m["phone_1"] == pytest.approx(0.5)
    assert config.scenario.source_distances_m["phone_2"] == pytest.approx(0.5)
    assert config.scenario.source_distances_m["phone_3"] == pytest.approx(0.7)
    assert config.scenario.source_distances_m["ap"] == pytest.approx(0.3)
    assert config.scenario.source_distances_m["ap_0"] == pytest.approx(0.3)
    assert config.wifi.medium_access_model == "shared_medium_serial"
    assert config.wifi.guard_time_s == pytest.approx(1.0e-5)


def test_duplicate_mobile_device_id_raises(tmp_path: Path) -> None:
    """Mobile device IDs must be unique in multi-mobile configs."""
    config_path = tmp_path / "duplicate_device.yaml"
    config_path.write_text(
        """
simulation:
  duration_s: 1.0
...
""",
    ...
    with pytest.raises(ValueError, match="duplicate device_id 'phone_1'"):
        load_config(config_path)


def test_negative_multi_mobile_distance_raises(tmp_path: Path) -> None:
    """AP and mobile distances in the new schema must be positive."""
    config_path = tmp_path / "negative_distance.yaml"
    config_path.write_text(
        """
simulation:
  duration_s: 1.0
...
""",
    ...
    with pytest.raises(ValueError, match="mobile_devices\\[0\\].distance_m"):
        load_config(config_path)
...
""",
    ...
    assert config.scenario.source_distances_m == {
        "mobile": 0.5,
        "ap": 0.3,
        "phone_1": 0.5,
    }
