...
def get_distance_for_source(source_id: str, scenario_config: ScenarioConfig) -> float:
    ...
    Preferred multi-mobile geometry is resolved first, then explicit legacy
    source mappings, and finally legacy single-distance fallbacks.
    """
    for device in scenario_config.mobile_devices:
        if source_id == device.device_id:
            return _validated_distance(
                device.distance_m,
                f"scenario.mobile_devices.{device.device_id}.distance_m",
            )
    if scenario_config.ap is not None and source_id == scenario_config.ap.ap_id:
        return _validated_distance(
            scenario_config.ap.distance_m,
    ...
    raise ValueError(
        ...
        "Set scenario.mobile_devices, scenario.ap, scenario.source_distances_m, "
        "or legacy scenario.distance_m."
    )
